...
int ranks_rows_cmp(const void *ii, const void *jj, void *arg) {
    ...
    const unsigned int j = *((unsigned int *) jj);
...
SEXP k_top_ranked_sets_r(SEXP ranks, SEXP k_max, SEXP min_max_freq, SEXP active) {
    ...
    unsigned int p = (unsigned int) INTEGER(ranks_dim)[0];
    unsigned int B = (unsigned int) INTEGER(ranks_dim)[1];

    unsigned int val_k_max = (unsigned int) INTEGER(k_max)[0];
    unsigned int val_min_max_freq = (unsigned int) INTEGER(min_max_freq)[0];
    ...
    SEXP subsets;
    PROTECT(subsets = allocVector(VECSXP, (unsigned int) val_k));
    SEXP freq;
    PROTECT(freq = allocVector(INTSXP, (unsigned int) val_k));
    no_protect += 2;
    SEXP subset;
    ...
    register unsigned int len_active = (unsigned int) length(active);
